...
def _get_backend() -> str:
    ...
    # available backend. crawl4ai (self-hosted, free) comes first so it
    # becomes the default on fresh installs with no paid keys. Firecrawl
    # also counts as available when the managed tool gateway is configured
    # for Nous subscribers.
    # Free-tier backends (searxng / brave-free / ddgs) trail the paid ones so
    # existing paid setups are unaffected.
    backend_candidates = (
        (
            "crawl4ai",
            _crawl4ai_provider_importable(),
        ),
        (
            "firecrawl",
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "crawl4ai":
        return _crawl4ai_provider_importable()
    if backend == "exa":
...
def _crawl4ai_provider_importable() -> bool:
    """Return True when the crawl4ai provider's core deps are available.

    The crawl4ai backend needs ``httpx`` + ``trafilatura`` as minimum
    deps (the Playwright-backed tier is optional).  Wrapped in a helper
    so auto-detect and ``_is_backend_available`` share the same check.
    """
    try:
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # Dispatch through the web search registry. When chain mode is
        # active (web.search_chain in config, or auto-chain from preference
        # order), we try each provider in order until one succeeds.
        # Falls back to single-provider dispatch when no chain exists.
        ...
        chain = resolve_search_chain()
        if chain and len(chain) > 1:
            # Chain mode: try each provider in order
            response_data = None
            last_error = None
            for provider in chain:
                logger.info(
                    "Web search chain: trying %s: '%s' (limit: %d)",
                    provider.name,
                    query,
                    limit,
                )
                try:
                    ...
                    logger.debug("Web search chain: %s raised: %s", provider.name, exc)
                    last_error = str(exc)
                ...
                logger.info(
                    "Web search chain: %s failed, trying next provider",
                    provider.name,
                )
                last_error = response_data.get("error", str(response_data))

            if response_data is None or _is_provider_failure(response_data):
                response_data = {
                    "success": False,
                    "error": last_error or "All search providers in chain failed",
                }
        else:
            # Single-provider dispatch (legacy path)
...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
            ...
            # Chain-aware extract dispatch: when the chain has multiple
            # extract-capable providers, try each in order. If the first
            # provider produces only errors (Cloudflare, 403, etc.),
            # the next provider (typically Firecrawl) gets a shot.
            import inspect

            from agent.web_search_registry import (
                get_active_extract_provider,
                get_provider as _wsp_get_provider,
                resolve_extract_chain,
                _is_provider_failure,
            )

            chain = resolve_extract_chain()
            results = None
            last_error = None

            if chain and len(chain) > 1:
                # Chain mode: try providers in order until one succeeds
                for extract_provider in chain:
                    logger.info(
                        "Web extract chain: trying %s: %d URL(s)",
                        extract_provider.name,
                        len(safe_urls),
                    )
                    try:
                        if inspect.iscoroutinefunction(extract_provider.extract):
                            provider_results = await extract_provider.extract(safe_urls, format=format)
                        else:
                            provider_results = await asyncio.to_thread(
                                extract_provider.extract, safe_urls, format=format
                            )
                    except Exception as exc:
                        logger.debug("Web extract chain: %s raised: %s", extract_provider.name, exc)
                        last_error = str(exc)
                        provider_results = None

                    if provider_results and not _is_provider_failure(provider_results):
                        results = provider_results
                        break

                    # Some URLs may have succeeded even if others failed —
                    # check if we got at least some useful content
                    if isinstance(provider_results, list):
                        success_count = sum(1 for r in provider_results if r.get("content") and not r.get("error"))
                        if success_count > 0:
                            results = provider_results
                            break

                    logger.info(
                        "Web extract chain: %s failed for all URLs, trying next provider",
                        extract_provider.name,
                    )
                    last_error = f"{extract_provider.name}: all URLs failed"

                if results is None:
                    results = [{"url": u, "title": "", "content": "",
                                "error": last_error or "All extract providers in chain failed"}
                               for u in safe_urls]
            else:
                # Single-provider dispatch (legacy path)
        ...
        if crawl_provider is not None:
            ...
            if use_llm_processing and auxiliary_available:
                ...
                for result_item in processed_results:
                    if isinstance(result_item, BaseException):
                        logger.warning(
                            "Tavily crawl processing task failed: %s", result_item
                        )
                        continue
                    result, metrics, status = result_item
                    if status == "processed":
